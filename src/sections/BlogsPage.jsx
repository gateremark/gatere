...
import AllBlogCard from "../components/cards/AllBlogCard";
import { Link } from "react-router-dom";
import { GiReturnArrow } from "react-icons/gi";
import { useState } from "react";
import { blogData } from "../components/Data/BlogsData";
import BlogCategory from "../components/BlogCategory";
import StarsCanvas from "../canvas/Stars";
import SpringyText from "../components/animationtext/SpringyText";
	...
	return (
		...
		>
			...
			<div className="pt-16 flex flex-col justify-center items-center font-medium">
				<SpringyText className="text-4xl mb-6 font-semibold" text="| Blogs" />
				<BlogCategory
				...
				<div className="px-5 md:px-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-start gap-6">
					{blogs.map((item, index) => (
						<AllBlogCard key={`blog-${index}`} {...item} index={index + 1} />
					))}
				</div>
			</div>
			<StarsCanvas />
